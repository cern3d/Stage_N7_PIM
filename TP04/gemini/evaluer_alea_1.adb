...
procedure Evaluer_Alea is
    ...
    procedure Calculer_Statistiques (
        Borne    : in Integer;  -- Borne supérieure de l'intervalle de recherche
    ...
    is
        ...
        -- 1. Définition du type tableau pour stocker les fréquences
        type Tableau_Frequences is array (1 .. Borne) of Integer;
        
        -- 2. Initialisation du tableau à 0 pour chaque face/valeur
        Frequences : Tableau_Frequences := (others => 0);
        
        Tirage : Integer;
    begin
        -- 3. Simulation des tirages et comptage
        for I in 1 .. Taille loop
            Get_Random_Number(Tirage);
            Frequences(Tirage) := Frequences(Tirage) + 1;
        end loop;

        -- 4. Recherche du Min et du Max dans le tableau de fréquences
        Min := Frequences(1);
        Max := Frequences(1);
        
        for I in 2 .. Borne loop
            if Frequences(I) < Min then
                Min := Frequences(I);
            end if;
        ...
        -- 5. Calcul de la moyenne théorique (imposée par la postcondition)
