...
procedure Tableau_Supprimer is
    ...
    -- Objectif : Supprimer tous les éléments négatifs du tableau en gardant l'ordre.
    -- Paramètre : Tab (en entrée/sortie)
    procedure Supprimer_Negatifs (Tab : in out T_Tableau) is
        Indice_Ecriture : Integer := 1;
    begin
        for I in 1 .. Tab.Taille loop
            if Tab.Elements (I) >= 0 then
                Tab.Elements (Indice_Ecriture) := Tab.Elements (I);
                Indice_Ecriture := Indice_Ecriture + 1;
            end if;
        end loop;
        
        -- Mettre à jour la nouvelle taille du tableau
        Tab.Taille := Indice_Ecriture - 1;
    end Supprimer_Negatifs;
