...
is
    ...
    -- Tableau des fréquences absolues pour chaque valeur 1..Borne
    Frequences : array (1 .. Borne) of Integer := (others => 0);
    Tirage     : Integer;

begin
    -- 1. Effectuer les tirages et compter les fréquences
    for I in 1 .. Taille loop
        Get_Random_Number (Tirage);  -- ✅ nom corrigé
        Frequences (Tirage) := Frequences (Tirage) + 1;
    end loop;

    -- 2. Calculer min et max des fréquences
    Min := Frequences (1);
    Max := Frequences (1);
    for J in 2 .. Borne loop
        if Frequences (J) < Min then
            Min := Frequences (J);
        end if;
        if Frequences (J) > Max then
            Max := Frequences (J);
        end if;
    end loop;

    -- 3. Calculer la moyenne théorique
...
--  x86_64-linux-gnu-gcc-13 -c -I./ -gnatwa -I- ./evaluer_alea.2.adb
--  evaluer_alea.2.adb:3:09: warning: no entities of "Ada.Integer_Text_IO" are referenced [-gnatwu]
--  evaluer_alea.2.adb:4:01: warning: use clause for package "Integer_Text_IO" has no effect [-gnatwu]
--  x86_64-linux-gnu-gnatbind-13 -x evaluer_alea.2.ali
--  x86_64-linux-gnu-gnatlink-13 evaluer_alea.2.ali
