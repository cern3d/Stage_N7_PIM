...
procedure Tableau_Supprimer is
    ...
    -- Objectif : Supprimer les éléments qui ne sont pas dans l'intervalle [0..20].
    -- Paramètre : Tab (en entrée/sortie)
    procedure Supprimer_Hors_Intervalle (Tab : in out T_Tableau) is
        Indice_Ecriture : Integer := 1;
    begin
        for I in 1 .. Tab.Taille loop
            if Tab.Elements (I) >= 0 and then Tab.Elements (I) <= 20 then
                Tab.Elements (Indice_Ecriture) := Tab.Elements (I);
                Indice_Ecriture := Indice_Ecriture + 1;
            end if;
        end loop;

        Tab.Taille := Indice_Ecriture - 1;
    end Supprimer_Hors_Intervalle;
    ...
    procedure Tester_Supprimer is
        ...
        begin
            Copie := Tab;
            Supprimer_Hors_Intervalle (Copie);
...
begin
    ...
    Supprimer_Hors_Intervalle (Tab1);
