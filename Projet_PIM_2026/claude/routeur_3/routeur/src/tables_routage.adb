...
package body Tables_Routage is
   ...
   -- Masque_Discriminant  --
   --------------------------
   -- Principe (§1.4.2) : mettre en cache avec le plus long masque présent
   -- dans la table, appliqué à Dest. Ainsi la destination mise en cache
   -- (Dest AND Masque_Res) ne peut pas être confondue avec une adresse
   -- pour laquelle une route plus spécifique existe.
   --
   -- Algorithme : parcourir toute la table et retenir le masque le plus long.
   -- Si aucun masque plus long que Long_Min n'existe, on garde Long_Min.
   procedure Masque_Discriminant (Table      :     T_Table;
      ...
      -- Construire un masque de N bits (N premiers bits à 1 depuis le MSB)
   ...
   begin
      -- Partir du masque de la route sélectionnée
      Long_Res   := Long_Min;
      Masque_Res := Masque_De_Long (Long_Min);

      -- Chercher le masque le plus long dans toute la table
      while Courant /= null loop
         Long_C := Longueur_Masque (Courant.Route.Masque);
         if Long_C > Long_Res then
            Long_Res   := Long_C;
      ...
      -- Dest n'est pas utilisé directement ici (la destination mise en cache
      -- est calculée par l'appelant via Dest AND Masque_Res).
