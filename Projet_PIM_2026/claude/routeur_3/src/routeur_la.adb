-- routeur_LA : routeur avec liste chaînée pour la table de routage
--              et arbre préfixe (trie) pour le cache.
--
-- Raffinage principal :
...
--   4. Initialiser le cache (trie, politique, taille)
--   5. Ouvrir le fichier de résultats en écriture
--   6. Traiter les paquets (cache → table, commandes inline)
...
with Caches_LL;         use Caches_LL;  -- pour T_Stats
with Caches_Trie;       use Caches_Trie;
with E_S_Fichiers_LA;
...
begin
   ...
   -- Étapes 2 & 3 : initialiser et charger la table de routage
   Initialiser (Table);
   E_S_Fichiers_LA.Lire_Table (Params.Fichier_Table.all, Table);

   -- Étape 4 : initialiser le cache trie
   ...
   E_S_Fichiers_LA.Traiter_Paquets (Params.Fichier_Paquets.all,
                                    Table, Cache, Fich_Res);
   ...
   -- Étape 8 : fermeture et libération
   Close (Fich_Res);
   Finaliser (Cache);
   Finaliser (Table);
...
exception
   when Ada.IO_Exceptions.Name_Error =>
      ...
      if Is_Open (Fich_Res) then Close (Fich_Res); end if;
      Finaliser (Cache);
      Finaliser (Table);
   ...
   when Constraint_Error =>
      Put_Line (Standard_Error,
                "Erreur d'argument. "
                & "Usage : routeur_LA [-c <n>] [-p FIFO|LRU|LFU] "
