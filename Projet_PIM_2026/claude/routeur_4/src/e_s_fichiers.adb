...
package body E_S_Fichiers is
   ...
   -- à partir de la position Pos. Avance Pos après le token.
   ...
   -- Afficher le séparateur de commande (ligne vide sauf pour la première)
      ...
      while not End_Of_File (Fichier) loop
         ...
         begin
            if S'Length = 0 then
               ...
               Caches_LL.Afficher (Cache);

            elsif S = "stat" then
               Sep_Commande (Premiere_Cmd);
               Put_Line ("stat (ligne" & Natural'Image (No_Ligne) & ")");
               Caches_LL.Afficher_Stats (Cache);
               ...
               begin
                  ...
                  Cache_Hit := Caches_LL.Chercher (Cache, Dest, Route);

                  if Cache_Hit then
                     -- Hit : utiliser directement la route du cache
                     Put_Line (Fichier_Res,
                               S & " "
                               & Trim (Route.Iface (1 .. Route.Iface_Len),
                                       Right));
                  else
                     -- Miss : chercher dans la table de routage
                     ...
                     if Table_Hit then
                        ...
                        Put_Line (Fichier_Res,
                                  ...
                                  & Trim (Route.Iface (1 .. Route.Iface_Len),
                                          Right));
                        -- Insérer dans le cache avec masque discriminant
                        Caches_LL.Inserer (Cache, Table, Dest, Route);
                     else
                        -- Aucune route : signaler l'erreur
