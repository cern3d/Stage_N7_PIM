...
with Pack_Cache_Arbre;      use Pack_Cache_Arbre; 
...
procedure Routeur_LA is
   ...
   -- Nettoyage de robustesse (Correction : S est désormais constante)
   ...
   while not End_Of_File(File_P) loop
      ...
      if Ligne_Lue = "table" then
         ...
         if not Cache_Hit then
            ...
            -- CORRECTION : Appel en adéquation avec votre signature Pack_Table_Routage
            Chercher_Route(Table, IP_Cible, Interf_Sortie); 
            
            -- Construction de la route pour le cache avec un masque /32 (hôte) 
            -- pour éviter tout conflit de cohérence sans toucher à votre Pack_Table_Routage.
            Nouvelle_Route_C.Destination := IP_Cible;
            Nouvelle_Route_C.Masque      := String_To_IP("255.255.255.255");
