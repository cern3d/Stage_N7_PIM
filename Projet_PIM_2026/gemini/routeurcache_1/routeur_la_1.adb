...
with Pack_Cache_Arbre;      use Pack_Cache_Arbre; -- On utilise désormais le cache en arbre
...
procedure Routeur_LA is
   ...
   -- Nettoyage de robustesse (CRLF Windows, espaces parasites)
   ...
   Route_Tmp        : T_Route;
   Route_Trouvee    : T_Route; -- Route complète extraite pour calcul du masque cache
   Cache_Hit        : Boolean;
   Nouvelle_Route_C : T_Route_Cache;
...
begin
   ...
   while not End_Of_File(File_P) loop
      ...
      if Ligne_Lue = "table" then
         ...
         -- 1. Recherche dans le cache en arbre
         Chercher_Cache(Cache, IP_Cible, Interf_Sortie, Cache_Hit);
         
         -- 2. En cas de défaut de cache, on interroge la table de routage
         if not Cache_Hit then
            Defauts_Cache := Defauts_Cache + 1;
            
            -- Remarque : s'assurer que Chercher_Route renvoie l'enregistrement complet Route_Trouvee
            Chercher_Route(Table, IP_Cible, Route_Trouvee); 
            Interf_Sortie := Route_Trouvee.Interface_R;
            
            -- Calcul de la route à mettre en cache conformément à la règle de cohérence du sujet :
            -- Destination = IP demandée masquée par le masque de la table
            Nouvelle_Route_C.Destination := IP_Cible and Route_Trouvee.Masque;
            Nouvelle_Route_C.Masque      := Route_Trouvee.Masque;
            Nouvelle_Route_C.Interface_R := Interf_Sortie;
            
            Ajouter_Cache(Cache, Nouvelle_Route_C);
         end if;
   ...
   -- Affichage final des statistiques si demandé
...
--  x86_64-linux-gnu-gcc-13 -c -gnatwa routeur_la_1.adb
--  routeur_la_1.adb:8:11: warning: file name does not match unit name, should be "routeur_la.adb" [enabled by default]
--  routeur_la_1.adb:25:07: warning: "S" is not modified, could be declared constant [-gnatwk]
--  routeur_la_1.adb:144:45: error: expected private type "Ada.Strings.Unbounded.Unbounded_String"
--  routeur_la_1.adb:144:45: error: found type "T_Route" defined at types_definitions.ads:22
--  gnatmake: "routeur_la_1.adb" compilation error