-- Routeur avec table en liste chaînée et cache en arbre préfixe (trie)
...
with Cache_Trie;          use Cache_Trie;
...
begin
   ...
   -- Init cache
   Cache_Trie.Init(Cache_Size, To_String(Policy_UB));
   ...
   while not End_Of_File(PFile) loop
      ...
      if Length(S_UB) = 0 then
         ...
         declare
            Req, Ht, Ms : Natural;
         begin
            Cache_Trie.Get_Stats(Req, Ht, Ms);
         ...
         if To_String(S_UB) /= "table" and To_String(S_UB) /= "cache" and 
            ...
            -- try cache
            Iface_UB := To_Unbounded_String(Trim_Line(Cache_Trie.Lookup(IP)));
               ...
               begin
                  if RR.Mask_Len = -1 then
                     ...
                     if L > 0 then
                        ...
                        Cache_Trie.Insert(IP, RR.Mask, RR.Mask_Len, Iface_Str(1..L));
